...
The home structure figure is generated by `structureFigure()` from `SECTIONS`. Two SVGs share that registry. Compact (`viewBox 0 0 480 380`, names in the bands) is the visible drawing below 80rem. Wide (`viewBox 0 0 1200 380`, leaders and roles) is the visible drawing from 80rem. The fallback list is visually hidden. It does not replace the SVG. No bitmap stands in for the drawing.
