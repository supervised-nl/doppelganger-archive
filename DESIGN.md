...
The home structure figure is generated by `structureFigure()` from `SECTIONS`. Two SVGs share one drawing. Compact (`viewBox 0 0 700 380`) is the visible drawing below 80rem. Wide (`viewBox 0 0 1200 380`) is the visible drawing from 80rem, with the role column. The fallback list is visually hidden. It does not replace the SVG. No bitmap stands in for the drawing.
